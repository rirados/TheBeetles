...
export default function AdminPage() {
  ...
  const clearRouteView = useCallback(() => {
    setPlannedRoutes([]);
  ...
  const handlePlanRoutes = useCallback(async (origin, destination, profile) => {
    ...
    try {
      ...
      const safePaths = paths.filter((route) => {
        const hasBlockedEdge = (route.blocked_edges || 0) > 0;
        const intersectsFlood = floodMarkers.some((fm) => {
          return (route.geometry || []).some((pt) => haversineM(fm.lat, fm.lng, pt[0], pt[1]) < 150);
        });
        return !hasBlockedEdge && !intersectsFlood;
      });
      const displayPaths = safePaths.length > 0 ? safePaths : paths;
      setPlannedRoutes(displayPaths);
      setRouteOverlayVisible(true);
      if (displayPaths.length > 0) {
        const bestIdx = displayPaths.reduce((best, r, i) => (r.total_risk < displayPaths[best].total_risk ? i : best), 0);
  ...
  const startSimulationOnGeometry = useCallback((geometry, startPoint = null) => {
  ...
  const handleStartSimulation = useCallback(() => {
    ...
    startSimulationOnGeometry(geom, { lat: geom[0][0], lng: geom[0][1] });
  ...
  const handleFloodSimulation = useCallback(async (lat, lng) => {
    ...
    for (let i = currentGeomIdx; i < geom.length; i++) {
      const dist = haversineM(lat, lng, geom[i][0], geom[i][1]);
    ...
    try {
      ...
      const bestRoute = paths[bestIdx];
      log("sim_rerouted", `REROUTED! ${paths.length} new routes computed in ${res.computation_ms}ms. Best: Route ${String.fromCharCode(65 + bestIdx)} (risk ${(bestRoute.total_risk * 100).toFixed(0)}%)`);

      startSimulationOnGeometry(bestRoute.geometry, newOrigin);
    } catch (e) {
      log("error", `Reroute failed: ${e.message}`);
      setSimulating(false);
    }
  }, [simulating, simVehiclePos, simDestination, simGeomIdx, startSimulationOnGeometry, floodMarkers, log]);
  ...
  }, [clearRouteView, handleFloodSimulation]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_460px] overflow-hidden">
        ...
        <div className="relative">
          <MapContainer center={DEFAULT_CENTER} zoom={14} className="h-full w-full">
            ...
            {routeOverlayVisible && plannedRoutes.map((route, idx) => {
