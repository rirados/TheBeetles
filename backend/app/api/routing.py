...
def plan_route(payload: schemas.RouteRequest, db: Session = Depends(get_db)):
    ...
    Supports profiles: fastest, safest, emergency, smart (default).

    The 'smart' profile uses a 4-factor weighted environmental risk:
    ...
    return schemas.RouteResponse(
        ...
        notes=f"Returned {len(results)} path(s) using profile '{payload.profile}'",
    )
