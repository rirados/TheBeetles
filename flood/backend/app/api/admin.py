...
def simulate_flood(lat: float, lng: float, depth: str = "waist"):
    ...
    Used by the Route Planner simulation to place a flood on the map
    and trigger real-time rerouting of simulated vehicles.
    """
    update = flood_intelligence.apply_report(
        lat=lat, lng=lng, depth_label=depth, confidence=0.95,
    )
    edges = []
    for (u, v, k), risk in update.risk_after.items():
        geom = road_network.edge_geometry(u, v, k)
        edges.append({
            "u": u, "v": v, "k": k,
            "risk": round(risk, 3),
            "blocked": update.blocked_after.get((u, v, k), False),
            "geometry": geom,
        })
    return {"ok": True, "affected_edges": edges}
